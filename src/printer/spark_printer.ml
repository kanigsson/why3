...
and print_tnode pri tables fmt t = match t.t_node with
  ...
  | Tbinop (b,f1,f2) ->
      ...
      if b = Timplies then
        fprintf fmt (protect_on (pri > p) "if %a@ then %a")
          (print_lterm (p + 1) tables) f1 (print_lterm p tables) f2
      else
...
let () = register_printer "spark" print_task
...
let () = register_printer "spark_itp" print_sequent
